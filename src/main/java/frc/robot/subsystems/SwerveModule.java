...
public class SwerveModule extends SubsystemBase {
  ...
  public SwerveModule(int moduleNum, TalonFX driveMotor, TalonFX turningMotor, CANCoder encoder, boolean invertDrive,
      boolean invertTurn, PIDController driveMotorPID, PIDController turnMotorPID, SimpleMotorFeedforward feedforward) {
  ...
  {
    double offset = 0.0;
    //double absolutePosition = Conversions.degreesToFalcon(Rotation2d.fromDegrees(e_Encoder.getAbsolutePosition())
  ...
  public SwerveModuleState getState() {
    return new SwerveModuleState(getVelocity(),
      Rotation2d.fromDegrees(Conversions.falconToDegrees(m_turningMotor.getSelectedSensorPosition(), C_TURNING_MOTOR_GEAR_RATIO)));
  }
